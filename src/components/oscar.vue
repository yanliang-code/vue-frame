<template>
  <div>
    ...
    <el-main :style="'height:' + contentHeight + 'px;padding: 25px 20px;'">
      <content1 v-show="nowProcess === 1" ref="content1"></content1>
      <content2 v-show="nowProcess === 2" ref="content2"></content2>
    </el-main>
...
export default {
  ...
  methods: {
    ...
    btnclick(isDown) {
      if (isDown) {
        let getData = this.$refs["content" + this.nowProcess].getData();
        if (!getData) {
          return;
        }
        console.log("this.stepData===", this.stepData);
        this.stepData["content" + this.nowProcess] = getData;
        this.nowProcess++;
      } else {
        this.nowProcess--;
      }
